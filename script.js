...
function drawScene3() {
  ...
  d3.csv("global_mmr_dashboard_ready.csv").then(raw => {
    ...
    function updateChart() {
      ...
      const x = d3.scaleLinear()
        .domain([0, d3.max(filtered, d => d.LiveBirths)]).nice()
        .range([80, width - 100]);
      ...
      svg.selectAll("circle")
        ...
        .attr("cy", d => y(d.MMR))
        .attr("r", d => 5 + (d.MMR / 100)) // scaled bubble size
        .attr("fill", "#3498db")
        .attr("opacity", 0.7)
        .on("mouseover", (event, d) => {
        ...
        .on("mouseout", (event, d) => {
          d3.select(event.currentTarget).transition().attr("r", d => 5 + (d.MMR / 100));
          tooltip.transition().duration(300).style("opacity", 0);
        })
        .on("click", (event, d) => {
